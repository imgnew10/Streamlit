...
for idx, agent in enumerate(agents_config):
    with agent_cols[idx]:
        ...
        if error:
            ...
            st.markdown(
                f"<div style='background-color:{agent['color']}15;border-radius:8px;padding:15px;margin:10px 0;border-left:4px solid {agent['color']};'>"
                f"<p style='margin:0;font-size:1.1rem;font-weight:bold;color:{agent['color']};'>{parsed['emoji']} {parsed['signal']}</p>"
                f"<p style='margin:12px 0 0 0;font-size:0.95rem;color:#444;line-height:1.6;white-space:pre-wrap;word-break:break-word;'>{parsed['reasoning']}</p>"
                f"</div>",
